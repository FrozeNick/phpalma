...
class FilterQueryBuilder {
    ...
    public function buildQuery($query, array $allowedFilters) {
        $allowed = $allowedFilters ? $allowedFilters : null;

        $filters = collect($this->request->query('filters'))->filter(function($rawFilter) use($allowed) {
          $operator = $this->getOperator($rawFilter);
          $filterName = explode($operator, $rawFilter)[0];
          
          return $allowed ? in_array($filterName, $allowed) : true;
        })->toArray();
